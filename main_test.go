...
func init() {
	...
	fmt.Printf("These are the varibles in the env: %v\n", os.Args[0])

	if strings.HasSuffix(os.Args[0], ".test") {
		fmt.Println("run under go test mine")
	} else {
...
func TestMain(m *testing.M) {
	...
	a = App{}
	a.Initialize(conn.DbUser, conn.DbPass, conn.DbName)
...
func clearTable() {
	//a is the application we want to test
	a.DB.Exec("DELETE FROM users")
	a.DB.Exec("ALTER TABLE users AUTO_INCREMENT = 1")
}

const tableCreationQuery = `CREATE TABLE IF NOT EXISTS users
	(id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(50) NOT NULL, age INT NOT NULL)`
...
func TestCreateUser(t *testing.T) {
	...
	if m["name"] != "test user" {
		t.Errorf("Expected username to be 'test user'. Got %v'", m["name"])
	}

	if m["age"] != 30.0 {
		t.Errorf("Expected user age to be '30'. Got '%v'", m["age"])
	}
	// the id is compared to 1.0 because JSON unmarshalling converts numbers to floats, when the target is a map[string]interface{}
	if m["id"] != 1.0 {
...
func addUsers(count int) {
	...
	//we convert an integer to string
	for i := 0; i < count; i++ {
